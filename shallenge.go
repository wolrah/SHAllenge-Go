...
func main() {
	...
	for sequence := start; sequence <= target; sequence++ {
		...
		//base64string := strings.Trim(base64.StdEncoding.EncodeToString(buf), "=")
		//slog.Debug("Base64 encoded", "sequence", sequence, "binary", buf, "base64string", base64string)
		//entry := username + "/" + prefix + base64string
		prefix := username + "/" + data
		entry := prefix + strconv.Itoa(sequence)
		sum := sha256.Sum256([]byte(entry))
		//slog.Debug("Hashed", "string", entry, "hash", sum)
		sum_slice := sum[:]
